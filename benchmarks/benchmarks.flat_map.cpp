 ...
 * Compares the ability of the layout to hide memory latency during binary
 * search.
 */
...
static void BM_Lookup_Random(benchmark::State& state)
{
  ...
  size_t       idx = 0;
  const size_t mask =
      n - 1; // Assuming n is power of 2 for fast wrapping, or just use %
...
#define BENCH_ARGS                                                             \
  ->RangeMultiplier(4)->Range(256, 32 << 20)->Unit(benchmark::kNanosecond)
