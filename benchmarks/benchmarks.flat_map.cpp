...
struct DataGenerator {
  static std::vector<T> generate(size_t n, uint64_t seed = 42) {
    ...
    if constexpr (std::is_integral_v<T>) {
      ...
      std::uniform_int_distribution<DistT> dist(std::numeric_limits<T>::min(), std::numeric_limits<T>::max());
...
struct OpBatchFind {
  ...
  template <typename Map, typename Needles, typename Results>
  static void run(const Map& map, const Needles& needles, Results& results) {
    map.batch_find(vault::amac::executor<kAMACBufferSize>, needles, std::back_inserter(results));
  }
};
...
struct OpBatchLowerBound {
  static std::string name() {
    return "Batch/LowerBound";
  }

  template <typename Map, typename Needles, typename Results>
  static void run(const Map& map, const Needles& needles, Results& results) {
    map.batch_lower_bound(vault::amac::executor<kAMACBufferSize>, needles, std::back_inserter(results));
  }
};
...
struct OpBatchUpperBound {
  static std::string name() {
    return "Batch/UpperBound";
  }

  template <typename Map, typename Needles, typename Results>
  static void run(const Map& map, const Needles& needles, Results& results) {
    map.batch_upper_bound(vault::amac::executor<kAMACBufferSize>, needles, std::back_inserter(results));
  }
};
...
#define ARGS_LOOKUP ->RangeMultiplier(4)->Range(256, 4 << 20)->Unit(benchmark::kNanosecond)

// Standard arguments for construction (can be slower, so smaller max range)
#define ARGS_CONSTRUCT ->RangeMultiplier(4)->Range(256, 1 << 18)->Unit(benchmark::kNanosecond)
