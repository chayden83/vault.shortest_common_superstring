...
static auto const haystack = ::ranges::actions::sort(randoms(25000000));
...
template <uint8_t N> void amac_lower_bound(benchmark::State& state)
{
  for (auto _ : state) {
    vault::amac::lower_bound<N>(haystack, needles, [&](auto&& job) {
