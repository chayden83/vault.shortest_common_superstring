...
namespace vault::amac {
  ...
  template <
    std::size_t FanoutBudget,
    typename TransitionProbability,
    typename StepRatio,
    std::size_t fanout_a,
    std::size_t fanout_b,
    typename TransitionFn>
  [[nodiscard]] constexpr auto make_pipeline(TransitionFn transition_fn) {
    static_assert(FanoutBudget >= fanout_a + fanout_b);

    auto step_ratio             = static_cast<double>(StepRatio::num) / StepRatio::den;
    auto transition_probability = static_cast<double>(TransitionProbability::num) / TransitionProbability::den;

    auto work_a = static_cast<double>(fanout_a);
    auto work_b = static_cast<double>(fanout_b) * transition_probability * step_ratio;

    auto budget_remaining = FanoutBudget - (fanout_a + fanout_b);

    auto budget_a = budget_remaining * work_a / (work_a + work_b);
    auto budget_b = budget_remaining * work_b / (work_a + work_b);

    auto extra_jobs_a = static_cast<std::size_t>(budget_a / fanout_a);
    auto extra_jobs_b = static_cast<std::size_t>(budget_b / fanout_b);

    auto window_a = 1 + extra_jobs_a;
    auto window_b = 1 + extra_jobs_b;

    auto whole_budget_a = window_a * fanout_a;
    auto whole_budget_b = window_b * fanout_b;

    if (whole_budget_a + whole_budget_b >= FanoutBudget) {
      return;
    }

    budget_remaining = FanoutBudget - whole_budget_a - whole_budget_b;

    while (budget_remaining != 0) {
      if (budget_remaining < fanout_a and budget_remaining < fanout_b) {
        return;
      } else if (budget_remaining < fanout_a) {
        while (budget_remaining >= fanout_b) {
          window_b += 1;
          budget_remaining -= fanout_b;
        }
      } else if (budget_remaining < fanout_b) {
        while (budget_remaining >= fanout_a) {
          window_a += 1;
          budget_remaining -= fanout_a;
        }
      } else {
        auto rem_a = (budget_a / static_cast<double>(fanout_a)) - static_cast<double>(extra_jobs_a);
        auto rem_b = (budget_b / static_cast<double>(fanout_b)) - static_cast<double>(extra_jobs_b);
