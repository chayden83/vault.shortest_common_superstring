...
template <
    ...
    typename C
>
concept OrderedLayoutPolicy = std::random_access_iterator<I> && std::copy_constructible<P> && requires
...
template <typename P>
concept ForwardLayoutPolicy = std::copy_constructible<P> && requires
...
template <typename P>
concept BidirectionalLayoutPolicy = ForwardLayoutPolicy<P> && requires
    (std::size_t n, std::ptrdiff_t idx)
{
