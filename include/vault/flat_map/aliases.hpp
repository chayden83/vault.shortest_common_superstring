...
namespace eytzinger::detail {
    ...
    [[nodiscard]] consteval std::size_t calculate_optimal_block_size() {
        ...
        // In standard C++, sizeof(K) is always a multiple of alignof(K), so this max() 
        // is theoretically redundant, but it protects against non-standard packed types
        // where sizeof might be smaller than the required alignment stride.
 ...
 * - Otherwise, B is calculated to fill one CPU cache line, respecting alignment.
 */
template <
    typename K, 
    typename V, 
