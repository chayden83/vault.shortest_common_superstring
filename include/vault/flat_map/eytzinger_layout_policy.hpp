...
public:
    ...
    public:
        ...
        void operator()(I first, S last) const {
            ...
            // Use iter_value_t to handle proxy iterators correctly (strips references)
    ...
    struct next_index_fn {
        constexpr std::ptrdiff_t operator()(std::ptrdiff_t i, std::size_t n_sz) const noexcept {
            ...
            if (right_child < n) {
                ...
                while (left_child < n) { i = left_child; left_child = (i << 1) + 1; }
                return i;
            } else {
                while (i > 0 && !(i & 1)) i = (i - 1) >> 1; 
                return (i > 0) ? ((i - 1) >> 1) : -1;
            }
        }
    };
    ...
    struct prev_index_fn {
        constexpr std::ptrdiff_t operator()(std::ptrdiff_t i, std::size_t n_sz) const noexcept {
            ...
            if (left_child < n) {
                i = left_child;
                auto right_child = (i << 1) + 2;
                while (right_child < n) { i = right_child; right_child = (i << 1) + 2; }
                return i;
            } else {
                while (i > 0 && (i & 1)) i = (i - 1) >> 1;
                return (i > 0) ? ((i - 1) >> 1) : -1;
