...
namespace eytzinger {
  ...
  /**
   * @brief A generic map container that decouples storage layout from
   * interface.
   * * @tparam LayoutPolicy Controls memory layout, permutation, and search
   * algorithms (e.g., eytzinger_layout_policy, sorted_layout_policy).
   */
  template <
      typename K,
      typename V,
  ...
  public:
    ...
    lower_bound(const K0& key) const noexcept
    {
      if (keys_.empty()) {
        return end();
      }
    ...
    template <typename K0 = key_type>
    [[nodiscard]] constexpr const_iterator
    upper_bound(const K0& key) const noexcept
    {
      if (keys_.empty()) {
        return end();
      }
