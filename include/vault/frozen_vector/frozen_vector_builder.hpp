...
namespace frozen {
  ...
  public:
    ...
    frozen_vector_builder(
        frozen_vector_builder&& other, const allocator_type& a
    )
        : allocator_(a)
        , size_(0)
        , capacity_(0)
    ...
    auto freeze() &&
    {
      return std::move(*this).template freeze<HandleTemplate<const T[]>>();
    }
    ...
    operator<=>(const frozen_vector_builder& other) const
    {
